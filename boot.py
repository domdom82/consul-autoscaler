...
def getBaseCmd():
  cmd=[ 'docker', 'run', '-d', '--net', 'host','--name', 'consul', '-h', HOSTNAME, '-e', 'AUTOSCALER_TYPE=' + TYPE,'-v', MYDIR + ':/docker-autoscaler','autoscaler-consul', 'agent', '-ui', '-advertise', HOSTIP, '-client=0.0.0.0', '-config-file=/docker-autoscaler/common/config/nanny-config.json' ]
  return cmd
